...
def fetch_biccamera_yamada_yodobashi_data(model_number_list,amazon_price_url_name_list):
    user_agent = [
    ...
    fetch_biccamera_yamada_yodobashi_cal_profit(biccamera_item_data,yamada_item_data,yodobashi_item_data,amazon_price_url_name_list)



def edion_kojima_nojima_cal_profit(jan_list,amazon_price_url_name_list):
    user_agent = [
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.159 Safari/537.36',
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.63 Safari/537.36',
    "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.69"
    ...
    kojima_item_data = fetch_kojima_data(driver,jan_list)
    nojima_item_data = fetch_nojima_data(driver,jan_list)
    driver.quit()
    
    edion_kojima_nojima_cal_profit(edion_item_data,kojima_item_data,nojima_item_data,amazon_price_url_name_list)
...
def main():
    ...
    p0 = multiprocessing.Process(target=fetch_biccamera_yamada_yodobashi_data,args=(model_number_list,jan_list,amazon_price_url_name_list))
    p1 = multiprocessing.Process(target=edion_kojima_nojima_cal_profit,args=(model_number_list,amazon_price_url_name_list))
